...
const deviceDetail = async (payload?: DeviceDetailPayload): Promise<deviceProps> => {
    try {
        const { data } = await axios.post(`api/v1/device/detail/list`, payload)
        return data
    } catch (err) {
        console.error("deviceDetail failed", {
            endpoint: "api/v1/device/detail/list",
            payload,
            error: err,
        });
        throw err;
...
export const useDeviceDetail = (payload?: DeviceDetailPayload) => {
    return useQuery({
        ...
        staleTime: 5 * 60 * 1000, // Cache for 5 minutes to avoid double-fetch
...
export const useDeviceDetailInfinite = (
  ...
  return useInfiniteQuery({
    ...
    getNextPageParam: (lastPage: deviceProps) => {
      ...
      // If we've already fetched all items, stop.
      if (page * pageLimit >= total) return undefined;
      return nextPage;
    },
    staleTime: 5 * 60 * 1000,
    refetchOnWindowFocus: false,
    retry: false, // Stop retrying automatically if the API errors.
  })
}