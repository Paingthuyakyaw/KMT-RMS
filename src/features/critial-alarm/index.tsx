...
import { Download, Filter, OctagonAlert } from "lucide-react";
...
const CritialAlarm = () => {
  ...
  const fakeCriticalAlarm = useMemo(
    () => [
      {
        deviceName: "MDY120",
        trigger: "Trigger A",
  ...
  const criticalCount = useMemo(
    () => fakeCriticalAlarm.filter((x) => x.alarmStatus === "Alarm").length,
    [fakeCriticalAlarm],
  );
  ...
  return (
    <div className="h-screen">
      <h4 className="text-sm font-semibold">Critical Alarm</h4>
      ...
      <div className="bg-white dark:bg-muted border rounded-md text-sm">
        ...
        {fakeCriticalAlarm.map((item, idx) => (
          <div
            key={idx}
